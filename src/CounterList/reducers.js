...
function removeCounter(countersState, index){
  return [
    ...countersState.slice(0, index),
    ...countersState.slice(index+1)
  ]
}
...
export const countersReducer = (countersState = [] , action) =>{
  switch(action.type){
    case ADD_COUNTER: return addCounter(countersState);
    case REMOVE_COUNTER: return removeCounter(countersState, action.index);
    case INCREMENT_COUNTER: return incrementCounter(countersState, action.index);
    case DECREMENT_COUNTER: return decrementCounter(countersState, action.index);
    default: return countersState;
  };
};
